...
async function ensureLoggedIn(page) {
  const dashboardUrl = 'https://aliciastorephone.go.scalenodes.app/dashboard/listing/products/add';
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: `${outDir}/product-add-page-v2.png` });
    fs.writeFileSync(`${outDir}/product-add-page-v2.html`, await page.content());
    ...
    const inputs = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('input, select, textarea')).map((i, idx) => ({
        ...
        closestText: i.closest('div, section, fieldset')?.textContent?.trim().substring(0, 80)
      }));
    });
    fs.writeFileSync(`${outDir}/product-add-inputs-v2.json`, JSON.stringify(inputs, null, 2));
    ...
    await page.screenshot({ path: `${outDir}/product-add-v2-error.png` });
    fs.writeFileSync(`${outDir}/product-add-v2-error.html`, await page.content());
  } finally {
    await browser.close();
  }
})();
