      ...
      {open && (
        <section className="mb-3 flex h-[min(560px,72vh)] w-[min(380px,calc(100vw-2.5rem))] flex-col border border-[#171811] bg-[#f4f0e8] shadow-[8px_8px_0_#171811]" aria-label="Shopping assistant">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-4" aria-live="polite">
            {lines.map((line, index) => <p key={`${line.role}-${index}`} className={`max-w-[88%] px-3 py-2 text-sm leading-6 ${line.role === "user" ? "ml-auto bg-[#171811] text-white" : "border border-[#171811]/20 bg-white"}`}>{line.content}</p>)}
            {loading && <p className="text-xs uppercase tracking-wider text-[#171811]/55">Thinking…</p>}
          </div>
